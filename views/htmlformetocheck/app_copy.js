  ...
  try {
      ...
      //Not working yet
          ...
          if (properties.length > 0) {
              ...
              res.status(200).json({ message: `No properties found for landlord with ID ${landlordId}` });
          }
      } else {
          ...
          if (properties.length > 0) {
              ...
              res.status(200).json({ message: "No properties found" });
          }
      }
  } catch (err) {
...
//check mongo return, if player found
  if (!application) {
      return res.status(200).send({ error: "application not found" });
    }
    //Ported for ease of understanding
